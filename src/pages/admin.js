      ...
      {messages.map(({ id, content, approved }) => (
        <li key={id} className="border-t p-3 flex items-center justify-between">
          ...
          {approved && (
            <button
              onClick={e => {
                rejectMessageOnClick(e, id);
              }}
              className="bg-red-500 p-3 rounded border text-white hover:bg-red-600 focus:bg-red-700"
            >
              Rechazar
            </button>
          )}
          {!approved && (
            <button
              onClick={e => approveMessageOnClick(e, id)}
              className="bg-white p-3 rounded border hover:bg-gray-100 focus:bg-gray-300"
            >
              Aprobar
            </button>
          )}
...
const AdminPage = () => {
  return (
    <div className="min-h-screen flex flex-col">
      <header>
        <h1 className="text-center font-bold uppercase">Liveblog Admin</h1>
      </header>
