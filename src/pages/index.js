...
const ApprovedMessageList = ({initialMessages}) => {
  ...
  useEffect(() => {
    if (approvedSubscription.data && !messages.map(m => m.id).find(m => m === approvedSubscription.data.messageApproved.id)) {
      setMessages([{...approvedSubscription.data.messageApproved}, ...messages.slice() ]);
    }
...
const IndexPage = () => {
  const { loading, error, data, refetch } = useQuery(utils.APPROVED_MESSAGES_QUERY, {notifyOnNetworkStatusChange: true});
  ...
  return (
    <div className="min-h-screen flex flex-col p-3">
      <header className="flex justify-between items-center">
        ...
        <button
          disabled={loading}
          className={`p-3 rounded border ${loading ? 'cursor-not-allowed bg-gray-500' : 'bg-white hover:bg-gray-100 focus:bg-gray-300'}`}
          onClick={() => {
            refetch();
          }}
        ><FaSyncAlt /></button>
      ...
      <section className="flex-1 flex flex-col">
        <section className="flex-1 flex p-3 overflow-y-scroll">
          {loading ? <p>Cargando...</p> :  <ApprovedMessageList initialMessages={data.approvedMessages} />}
        </section>
      </section>
    </div>
  );
